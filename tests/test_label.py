...
class LabelTest(UITestCase):
    ...
    def test_ui_extents(self):
        ...
        self.assertAlmostEqual(20.02, ui.extents(label).width, delta=TextTolerance)
        self.assertAlmostEqual(7.227, ui.extents(label).height, delta=TextTolerance)

        label.text_size = 15
        label.validate()

        self.assertAlmostEqual(30.03, ui.extents(label).width, delta=TextTolerance)
        self.assertAlmostEqual(10.840, ui.extents(label).height, delta=TextTolerance)
    ...
    def test_minimum_size(self):
        def test_with_padding(padding: Insets):
            with Label(self.context) as label:
                ...
                self.assertEqual(Nothing, label.minimum_size_override)
                self.assertAlmostEqual(20.02 + pw, label.minimum_size.width, delta=TextTolerance)
                self.assertAlmostEqual(7.227 + ph, label.minimum_size.height, delta=TextTolerance)
                ...
                self.assertEqual(Nothing, label.minimum_size_override)
                self.assertAlmostEqual(30.03 + pw, label.minimum_size.width, delta=TextTolerance)
                self.assertAlmostEqual(10.840 + ph, label.minimum_size.height, delta=TextTolerance)
    ...
    def test_preferred_size(self):
        def test_with_padding(padding: Insets):
            with Label(self.context) as label:
                ...
                self.assertEqual(Nothing, label.preferred_size_override)
                self.assertAlmostEqual(20.02 + pw, label.preferred_size.width, delta=TextTolerance)
                self.assertAlmostEqual(7.227 + ph, label.preferred_size.height, delta=TextTolerance)
                self.assertEqual(2, len(calculated))
                self.assertAlmostEqual(20.02 + pw, calculated[1].width, delta=TextTolerance)
                self.assertAlmostEqual(7.227 + ph, calculated[1].height, delta=TextTolerance)
                ...
                self.assertEqual(Nothing, label.preferred_size_override)
                self.assertAlmostEqual(30.03 + pw, label.preferred_size.width, delta=TextTolerance)
                self.assertAlmostEqual(10.840 + ph, label.preferred_size.height, delta=TextTolerance)
                self.assertEqual(3, len(calculated))
                self.assertAlmostEqual(30.03 + pw, calculated[2].width, delta=TextTolerance)
                self.assertAlmostEqual(10.840 + ph, calculated[2].height, delta=TextTolerance)
